...
#### Información de Hospitales en Quito
...
quito <- getbb("Quito", "Ecuador")
query <- opq(quito) %>% add_osm_feature(key="amenity", value="hospital") %>% 
  osmdata_sf()
...
geodatabase_path <- "./GEODATABASE_NACIONAL_2021/GEODATABASE_NACIONAL_2021.gdb/"
...
poblacion <- read_excel(.path_pob, sheet="1.2", range="B13:g1299")
# Renombrar variables
names(poblacion) <- c("provincia", "canton", "parroquia", "total_nacional", "hombres", "mujeres")

# trabajo la data de la base 
poblacion <-poblacion %>% filter(!is.na(provincia), str_detect(parroquia, "^Total|Nacional", negate=TRUE))
poblacion <- poblacion %>% 
...
dpa <- read_excel("./dpa_.xls",
                  sheet = "DPA_Base",
                  range = "B4:G1028")

names(dpa) <- c("cod_prov", "cod_canton", "cod_parr", "provincia", "canton", "parroquia")

dpa <- 
  dpa %>% 
  mutate(
    parroquia = ifelse(parroquia == "QUITO DISTRITO METROPOLITANO, CABECERA CANTONAL, CAPITAL PROVINCIAL Y DE LA REPUBLICA DEL ECUADOR",
                       "QUITO",
                       parroquia),
        parroquia = ifelse(parroquia == "CHECA (CHILPA)",
                           "CHECA",
                       parroquia),
    parroquia = ifelse(parroquia == "CALDERON (CARAPUNGO)",
                           "CALDERON",
                       parroquia),
    parroquia = ifelse(parroquia == "ATAHUALPA (HABASPAMBA)",
                           "ATAHUALPA",
                       parroquia)
      )
```
...
poblacion <- poblacion %>% 
  left_join(dpa %>% filter(cod_prov=="17"), by=c("parroquia"="parroquia"))

poblacion <-
  poblacion %>% 
  mutate(DPA_PARROQ = paste0(cod_prov,
                              cod_canton,
                              cod_parr))
...
Los resultados sugieren que en Quito, las amenidades de salud, específicamente los hospitales, tienden a estar concentradas en ciertas áreas geográficas. Esto podría reflejar una planificación o una distribución que favorece la proximidad de estas infraestructuras en determinadas zonas, lo que podría tener implicaciones para el acceso a servicios de salud en la región.

\#### Análisis de Autocorrelación Espacial con Matriz de Distancia Inversa

En esta sección, se analiza cómo cambia el índice de Moran $I$ al aplicar una matriz de pesos espaciales basada en la distancia inversa, con una distancia máxima de 1000 metros desde el centroide de cada zona censal.
...
sec_cens <- sec_cens %>% 
  mutate(centro=if_else(zon=="170150167",
                        true="Homme",
                        false=NA_character_))
